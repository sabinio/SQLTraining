...
AS
BEGIN
    DECLARE @Value INT, @CustomerID INT, @PriorStatus CHAR(1)
    BEGIN TRANSACTION
        BEGIN TRY
            IF (NOT EXISTS(SELECT [CustomerID] from [CustomerOrders] WHERE [OrderID] = @OrderID))
            BEGIN
            ...
            SELECT @Value = [Amount], @CustomerID = [CustomerID], @PriorStatus = [Status]
            ...
            BEGIN
                ...
                -- and then remove the amount from the CustomerOrders for the customer
                UPDATE [Customer]
                       SET
